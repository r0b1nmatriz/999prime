        ...
        img.onload = () => {
          ...
          try {
            ...
            const safeWidth = Math.max(
              1,
              Math.min(width, canvas.width - safeX),
            );
            const safeHeight = Math.max(
              1,
              Math.min(height, canvas.height - safeY),
            );
    ...
    <div className="relative w-full h-dvh flex flex-col items-center justify-center bg-black">
      ...
      <div className="absolute bottom-[100px] text-center z-10">
        <p className="font-mono text-gray-400 text-xs sm:text-base md:text-sm">
          <span className="text-gray-300 hover:text-silver-400 transition-colors duration-300">
            Skeet the Horizon-Energy
          </span>{" "}
          <span className="transition-colors duration-300">& Cosmic Buzz</span>
        </p>
      </div>
    </div>
  );
